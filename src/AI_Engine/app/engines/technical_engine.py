        ...
        try:
            ...
            try:
                ...
                if not intraday.empty:
                    ...
                    # Veriler değiştiği için indikatörleri tekrar hesapla
                    df = self._calculate_indicators(df)
                else:
            ...
            if llm_direction == "Up":
                # RSI: 65 altında alım fırsatı var, 75 üstü şişmiş
                if rsi_value < 65:
                    score += 2
                elif rsi_value > 75:
                    score -= 2
                ...
                # MACD: Yön yukarı, momentum da yukarıysa onay
                if is_macd_bullish:
                    score += 1

                # Hacim: Güçlü hacim yukarı trendi onaylar
                if volume_trend == "Strong":
                    score += 1

            elif llm_direction == "Down":
                # RSI: 35 üstündeyse hâlâ düşme potansiyeli var
                # 30 altı aşırı satılmış = düşüşü onaylamaz, toparlanma riski var
                if rsi_value > 35:
                    score += 2
                elif rsi_value < 30:
                    score -= 2
                ...
                # MACD: Yön aşağı, momentum da aşağıysa onay
                if not is_macd_bullish:
                    score += 1

                # Hacim: Güçlü hacimle düşüş = daha güvenilir sinyal
                if volume_trend == "Strong":
                    score += 1
